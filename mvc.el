...
(defcustom mvc-mvc-default-option-list '((diff . ((mercurial . nil)
					 ...
					 (status . ((mercurial . nil)
						    ...
						    (subversion . nil)
						    (cvs . nil)))
					 ;; ネットワークドライブなどで repository/working directory が遅い場合、
...
(defun mvc-log-mode-status-mode-next-status ()
  ...
  (catch 'mapc
    (mapc #'(lambda (a)
	      (let ((list (get-buffer-window-list a)))
		(when list
		  (with-selected-window (nth 0 list)
		...
		(let ((check-file-name buffer-file-name))
		  (with-current-buffer a
		    (when (and (boundp 'mvc-l-status-buffer-name-list)
		      ...
		      (let* ((tmpkey (concat "  " (substring check-file-name (length (expand-file-name default-directory)))))
		      	     (tmppoint (gethash tmpkey mvc-l-status-point-hash)))
		      	(save-excursion
		      	  (goto-char (+ tmppoint 3))
		      	  (setq buffer-read-only nil)
		      	  (delete-char 1)
		      	  (mvc-insert-with-face "m" 'mvc-face-status-modified)
		      	  (setq buffer-read-only t))))))))
	  (buffer-list))))
