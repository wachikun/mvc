...
'commit (argument(string):テンポラリログファイル名)"
  (let ((option-list (append (list (symbol-name command))
			     (cdr (assq mvc-l-status-program
					(cdr (assq command mvc-default-option-list)))))))
    (cond ((eq command 'log)
...
(defun mvc-status-mode-toggle-display-unknown ()
  ...
  (if (mvc-status-async-p)
      (message mvc-message-process-already-running)
    (if mvc-l-status-display-unknown-p
	(setq mvc-l-status-display-unknown-p nil)
      (setq mvc-l-status-display-unknown-p t))
...
(defun mvc-status-mode-toggle-display-unmodified ()
  ...
  (if (mvc-status-async-p)
      (message mvc-message-process-already-running)
    (if mvc-l-status-display-unmodified-p
	(setq mvc-l-status-display-unmodified-p nil)
      (setq mvc-l-status-display-unmodified-p t))
...
(defun mvc-status-mode-toggle-display-backup ()
  ...
  (if (mvc-status-async-p)
      (message mvc-message-process-already-running)
    (if mvc-l-status-display-backup-p
	(setq mvc-l-status-display-backup-p nil)
      (setq mvc-l-status-display-backup-p t))
...
(defun mvc-status-mode-toggle-display-ignore ()
  ...
  (if (mvc-status-async-p)
      (message mvc-message-process-already-running)
    (if mvc-l-status-display-ignore-p
	(setq mvc-l-status-display-ignore-p nil)
      (setq mvc-l-status-display-ignore-p t))
...
(defun mvc-status-mode-toggle-recursive ()
  ...
  (if (mvc-status-async-p)
	...
	(progn
	  (if mvc-l-status-recursive-p
	      (setq mvc-l-status-recursive-p nil)
	    (setq mvc-l-status-recursive-p t))
...
(defun mvc-log-mode-match-revision-line (limit)
  (or (re-search-forward "^\\([^:]+\\): +[0-9]+:\\([0-9a-f]+\\)$" limit t)
...
(defun mvc-log-mode-match-revision-line-backward (limit)
  (or (re-search-backward "^\\([^:]+\\): +[0-9]+:\\([0-9a-f]+\\)$" limit t)
      (re-search-backward "^[\\*\\|\\\\ ]*\\(commit\\) +\\([0-9a-f]+\\)" limit t)
      (re-search-backward "^\\(r\\)\\([0-9]+\\) +|" limit t)
      (re-search-backward "^\\(revno\\): \\([0-9]+\\)" limit t)))
	...
	(message "comment line!")
	  ...
	  (cond ((string-match "^#@elisp-eval" special)
		...
		((string-match "^#@script-eval" special)
		 (let ((process-bufname (mvc-create-buffer-name "*mvc-cheat-sheet-script-process*" default-directory))
		       (tmpfile (make-temp-name (concat mvc-default-tmp-directory "/mvcel-script")))
		       process)
		   (with-temp-file tmpfile
		     (insert command))
		   (set-file-modes tmpfile 448) ; 0700
		   (setq process (start-process-shell-command process-bufname
							      process-bufname
							      tmpfile))
		   ...
		   (switch-to-buffer-other-window process-bufname)))
		(t
		 (with-current-buffer mvc-l-cheat-sheet-mode-status-buffer
		   (let ((status-buffer (current-buffer))
			 async-process-buffer-name
			   ...
			   (t
				...
				(setq mode-line-process " command async"))
			      ...
			      (cond ((or (string-match "diff-mode" special)
					 (string-match "\s*[%a-z]+\s+diff" command))
		...
		(let ((check-file-name buffer-file-name))
		  (with-current-buffer a
		    (when (and (boundp 'mvc-l-status-buffer-name-list)
			       ...
			       (gethash check-file-name mvc-l-status-after-save-hook-hash))
