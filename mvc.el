...
;; ファイル数が多いワーキングコピーを指定しておくと status の待ち時間が短かくなります。
;; 下記フラグが制御されます。
...
(defun mvc-status-mode-toggle-display-delete ()
  ...
  (if (mvc-status-async-p)
      (message mvc-message-process-already-running)
    (if mvc-l-status-display-delete-p
	(setq mvc-l-status-display-delete-p nil)
      (setq mvc-l-status-display-delete-p t))
